...
### How do I update DClaw Learn?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp learn --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Learn without Kubernetes?

Yes, for local development:

```bash
cd dclaw-learn/frontend && npm run dev
cd dclaw-learn/backend && uvicorn dclaw_learn.main:app --reload
```
