...
kubectl exec -n dclaw-learn deployment/dclaw-learn-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-learn-db-credentials -n dclaw-learn
   ```
